...
JWT_ACCESS_EXPIRES_IN="${JWT_ACCESS_EXPIRES_IN:-12h}"
...
# Sem isto no .env, o compose interpolava NVIDIA_VISIBLE_DEVICES="" e o toolkit
# escondia a GPU mesmo com runtime nvidia (Dockerfile ENV era sobrescrito).
NVIDIA_VISIBLE_DEVICES="${NVIDIA_VISIBLE_DEVICES:-all}"
NVIDIA_DRIVER_CAPABILITIES="${NVIDIA_DRIVER_CAPABILITIES:-compute,utility}"
USE_GPU="${USE_GPU:-1}"
ALLOW_CPU_FALLBACK="${ALLOW_CPU_FALLBACK:-0}"
TORCH_NUM_THREADS="${TORCH_NUM_THREADS:-2}"
WORKER_CPUS="${WORKER_CPUS:-3.0}"
WORKER_MEM_LIMIT="${WORKER_MEM_LIMIT:-8g}"
